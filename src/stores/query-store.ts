...
function parseQueryId(id: string): { projectId: string; database: string; driver: string; title: string } | null {
  const parts = id.split(":");
  if (parts.length < 4) return null;
  return { projectId: parts[0], database: parts[1], driver: parts[2], title: parts.slice(3).join(":") };
}
...
interface QueryStore {
  ...
  saveQuery: (projectId: string, database: string, driver: string, title: string, sql: string) => Promise<void>;
...
export const useQueryStore = create<QueryStore>((set) => ({
  ...
  saveQuery: async (projectId, database, driver, title, sql) => {
    ...
    set((s) => ({
      queries: [...s.queries.filter((q) => q.id !== id), { id, projectId, database, driver, title, sql }],
    }));
  },
