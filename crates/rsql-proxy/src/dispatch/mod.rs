...
async fn route(
    ...
    match cmd {
        "project_db_select" | "project_db_insert" | "project_db_delete"
        | "query_db_select" | "query_db_insert" | "query_db_delete"
        | "workspace_save" | "workspace_load_all" | "workspace_delete" => {
            state_cmds::handle(session, cmd, payload, id).await
        }
        "system_resource_usage" | "compute_diff" => {
            utils_cmds::handle(session, cmd, payload, id).await
        }
        "pgsql_run_query" | "pgsql_run_query_packed" | "pgsql_execute_virtual"
        | "pgsql_fetch_page" | "pgsql_close_virtual" | "pgsql_cancel_query"
        | "pgsql_run_query_streamed" => {
            pgsql_query::handle(session, cmd, payload, id).await
        }
        "pgsql_listen_start" | "pgsql_listen_stop"
        | "pgsql_notify_send" | "pgsql_discover_channels" => {
            pgsql_pubsub::handle(session, cmd, payload, id).await
        }
