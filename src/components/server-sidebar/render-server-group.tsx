 ...
 */
export function renderServerGroup(
  ctx: SidebarRenderCtx,
  fp: string,
  pids: string[],
) {
  const {
    projects, status, serverDatabases, serverTablespaces,
    isOpen, toggle, onConnect, addDatabaseToServer, deleteProject, refreshConnection,
    openTab, openMonitorTab, openNotifyTab, openRolesTab, openSchemaDiffTab,
    openExtensionsTab, openEnumsTab, openPgSettingsTab,
    setAddDbSource, showMenu, copy, onEditConnection,
  } = ctx;
    ...
    <div key={gKey}>
      <TreeRow
        ...
        onContextMenu={(e) => showMenu(e, [
          { header: "Server" },
          ...(connectedPid ? [
            { label: "New Query", icon: <Plus className="h-3 w-3" />, onClick: () => openTab(connectedPid) },
            { label: "Performance Monitor", icon: <Activity className="h-3 w-3" />, onClick: () => openMonitorTab(connectedPid) },
            { label: "PG Settings", icon: <Settings className="h-3 w-3" />, onClick: () => openPgSettingsTab(connectedPid) },
          ] : []),
          { label: "Add Database", icon: <Plus className="h-3 w-3" />, onClick: () => setAddDbSource(pids[0]) },
          ...(onEditConnection ? [{ label: "Edit Connection", icon: <Edit3 className="h-3 w-3" />, onClick: () => onEditConnection(pids[0]) }] : []),
          { separator: true as const },
          { label: "Copy Name", icon: <Copy className="h-3 w-3" />, onClick: () => copy(serverLabel) },
          { separator: true as const },
          { label: "Delete", icon: <Trash2 className="h-3 w-3" />, onClick: () => { for (const pid of pids) void deleteProject(pid); }, destructive: true },
        ])}
        trailing={
      ...
      {isOpen(gKey, true) && (
        <>
          ...
          {isOpen(dbCatKey, true) && allDbs.map((dbName) => {
            ...
            if (dbPid) {
                ...
                <div key={dbName}>
                  <TreeRow
                    ...
                    onContextMenu={(e) => showMenu(e, [
                      ...
                      { separator: true as const },
                      { label: "Copy Name", icon: <Copy className="h-3 w-3" />, onClick: () => copy(dbName) },
                      { separator: true as const },
                      { label: "Delete", icon: <Trash2 className="h-3 w-3" />, onClick: () => void deleteProject(dbPid), destructive: true },
                    ])}
                    trailing={
                      <div className={cn("h-2 w-2 rounded-full shrink-0",
              ...
              return (
                <TreeRow
                  ...
                  onContextMenu={(e) => showMenu(e, [
                    { label: "Connect", icon: <Link2 className="h-3 w-3" />, onClick: () => void addDatabaseToServer(pids[0], dbName, dbName) },
                    { label: "Copy Name", icon: <Copy className="h-3 w-3" />, onClick: () => copy(dbName) },
                  ])}
                />
              );
          ...
          <TreeRow
            ...
            onClick={() => {
              if (connectedPid) { openRolesTab(connectedPid); }
              else { void onConnect(pids[0]).then(() => { const p = pids.find((id) => status[id] === ProjectConnectionStatus.Connected); if (p) openRolesTab(p); }); }
            }}
          />
            ...
            return (
              <>
                <TreeRow
                  ...
                  onClick={() => {
                    if (connectedPid) { toggle(tspCatKey); }
                    else { void onConnect(pids[0]); }
                  }}
                />
