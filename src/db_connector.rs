...
pub fn DBConnector() -> impl IntoView {
    ...
    view! {
        <div class="flex flex-row justify-between p-4 gap-2 border-b-1 border-neutral-200">
            ...
            <input class="border-1 border-neutral-200 p-1 rounded-md" type="text" value=move || db.db_port.get() placeholder="port" on:input=move |e| {
            db.db_port.set(event_target_value(&e)); } />
            </div>
            <button class="px-4 py-2 border-1 border-neutral-200 hover:bg-neutral-200 rounded-md" on:click=move |_| {
                connect.dispatch(db);
            } >Connect</button>
        </div>
    }
